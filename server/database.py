...
def create_song(song_name, tracks_json):
    ...
    vars["cursor"].execute(
        """INSERT INTO songs (SongName, Created, Updated, TracksJson)
                      VALUES (?,?,?,?)""",
        (song_name, current_time, current_time, tracks_json),
    )
    vars["conn"].commit()
    vars["cursor"].execute(
        "SELECT SongID FROM songs WHERE Created = ? and SongName = ? ",
        (current_time, song_name),
    )
        ...
        vars["cursor"].execute(
            """UPDATE songs
                          SET SongName = ?,
                              ...
                              TracksJson = ?
                          WHERE SongID = ?""",
            (song_name, current_time, tracks_json, song_id),
        )
        vars["conn"].commit()
...
def delete_song(song_id):
    vars["cursor"].execute(
        "SELECT SongID FROM songs WHERE SongID = ?", (song_id,)
    )
    result = vars["cursor"].fetchall()
    if len(result) == 1:
        vars["cursor"].execute(
            """DELETE FROM songs WHERE SongID = ?""", (song_id,)
        )
        vars["cursor"].execute(
            """DELETE FROM song_user_links WHERE SongID = ?""", (song_id,)
        )
        vars["conn"].commit()
        return True
    else:
...
def get_song_by_id(song_id):
    if song_id_exists(song_id):
        vars["cursor"].execute(
            "SELECT * FROM songs WHERE SongID = ? ", (song_id,)
        )
        result = vars["cursor"].fetchone()
        return strings2dict(
    ...
    if not (is_user_name_free(user_name)):
        ...
        for song in result:
            list_of_songs.append(
                strings2dict(song[0], song[1], song[2], song[3], song[4])
            )
...
def song_id_exists(song_id):
    vars["cursor"].execute(
        "SELECT count(SongID) FROM songs WHERE SongID=?", (song_id,)
    )
...
def is_user_name_free(user_name):
    vars["cursor"].execute(
        "SELECT count(UserName) FROM users WHERE UserName=?", [user_name]
    )
...
def check_user_token(user_name):
    vars["cursor"].execute(
        "SELECT Token, RefreshDate FROM tokens WHERE UserName=?", [user_name]
    )
    result = vars["cursor"].fetchall()
    if len(result) > 0:
        count = 0
