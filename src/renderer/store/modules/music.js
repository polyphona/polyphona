...
import { Track, SCALE, INVERSESCALE, Note, TrackLoader, clip } from '@/models'
import fs from 'fs'
...
class RenderContext {
  ...
  get percentPerInterval () {
    // NOTE: `clip` is a hotfix for rounding errors that would otherwise
    // cause notes to sometimes fall on the wrong lane.
    return clip(100 / Object.keys(this.musicContext.scale).length, 0.01)
  }
}
...
const mutations = {
  ...
  SCHEDULE_NOTES (state) {
    state.currentTrack.notes.forEach(note => {
      const pitch =
        state.musicContext.scale[note.pitch] + Number(state.musicContext.octave)
...
const actions = {
  ...
  async saveTrack ({ state, commit }) {
    ...
    const { data: res } = state.currentTrack.remoteId
      ? await http.put('songs/' + state.currentTrack.remoteId, data)
      : await http.post('songs', data)
    commit('SAVE', res)
  },
  async getSavedTracks ({ state, commit, rootState }) {
    const { data: res } = await http.get(
      'users/' + rootState.auth.user.username + '/songs'
    )
  ...
  exportMidi ({ dispatch, state }) {
    ...
    const toMidiTime = canvasTime =>
      canvasTime / state.musicContext.division / (Tone.Transport.bpm.value / 60)

    state.currentTrack.notes.forEach(note => {
      const pitch =
        state.musicContext.scale[note.pitch].toLowerCase() +
        state.musicContext.octave.toString()
      track.note(
        pitch,
      ...
      if (!err) {
        ...
        for (var i = 0; i < midiNotes.length; i++) {
          ...
          context.commit(
            'ADD_NOTE',
            new Note(
              toCanvasTime(midiNote.time),
              toCanvasTime(midiNote.duration),
              inverseScale[toCanvasPitch(name)[0]],
              midiNote.velocity
            )
          )
        }
